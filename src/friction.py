...
for j in range(len(file_name)):
    ...
    x,y,yfit = fit_line(np.array(time), np.array(jnt_ctrl_torque))
...
#     #qdot_1[0] = reduce(lambda x, y: math.exp(-(x/vs)**2) + y, qdot_1)
#     #qdot_2 x_dot_dot
#     exponential = reduce(lambda x, y: math.exp(-(x/vs)**2) + y, qdot_1)
#     g_v = Fc + (Fs - Fc) * exponential #* math.exp(-(qdot_1[0] / vs)**2)
...
# def error_2(poly_params, xs, ys, vs, Fc, Fs, s2, ts, t_range, force_rate):
#     qdot_3 = 0     # z_dot
#     qdot_1 = [0]     # x_dot
#     q_3 = 0        # z
#     q_1 = 0        # x
...
# #FUNCTION TO FIT THE PARAMETERS WITH THE ITERATIVE NEWTON METHOD
# def newton_3(poly_params, xs: np.ndarray, ys: np.ndarray, param_value_guess: np.ndarray,
#     vs, Fc, Fs, s2, t_range, ts, force_rate, max_iter: int=500, epsilon: float=0.001) -> Tuple[np.ndarray, int, float]:
#     finalErrorsList = []
...
#     num_eqns = len(poly_params)
#     num_data = len(ys)
#     #errorf = error_3(poly_params, xs, ys, vs, Fc, Fs, s2, time)
#     errorf = error_2(poly_params, xs, ys, vs, Fc, Fs, s2, ts, t_range, force_rate)
#     X = sp.Matrix([errorf])
#     Y = sp.Matrix(poly_params)
#     myjac = X.jacobian(Y)
#     xcurr = sp.Matrix(params)
...
#         for i,elem in enumerate(poly_params):
#             param_dict[elem] = xcurr[i]    
#         #final_error = error_3(poly_params, xs, ys, vs, Fc, Fs, s2, time).subs(param_dict)
#         final_error = error_2(poly_params, xs, ys, vs, Fc, Fs, s2, ts, t_range, force_rate).subs(param_dict)
#         #print(final_error)
#         if final_error <= least_error:
#             least_error = final_error
#             params_final = np.array(list(param_dict.values()),dtype=float)
...
# max_force = 1
# parameter_dyn, final_error_dyn = newton_3(poly_params_dyn, velocity, force, init_param_values, vs, Fc, Fs, sigma_2, t_sol, ts, max_force) 
