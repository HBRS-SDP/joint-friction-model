...
bool friction_controller::cyclic_torque_control(k_api::Base::BaseClient* base, k_api::BaseCyclic::BaseCyclicClient* base_cyclic, k_api::ActuatorConfig::ActuatorConfigClient* actuator_config)
{
    ...
    std::vector<double> zero_configuration, friction_configuration_joint_7;
    ...
    for(vector_interartor=0;vector_interartor<7;vector_interartor++)
    {
        ...
        friction_configuration_joint_7.push_back(root["friction_configuration_joint_7"][vector_interartor].As< double >());
    ...
    try {
        if (arm_position_configuration != "Home" &&
            ...
            arm_position_configuration != "friction_configuration_joint_7")
        {   
        ...
        for(int iteration_counter =0; iteration_counter<iteration_limit; iteration_counter++)
        {
            ...
            while (total_time_sec.count() / SECOND < task_time_limit_sec)
            {
                ...
                for (unsigned int i = 0; i < ACTUATOR_COUNT; i++)
                {
                    ...
                    {
                        if (i != TEST_JOINT) base_command.mutable_actuators(i)->set_position(friction_configuration_joint_7[i]);
                    }
                }
