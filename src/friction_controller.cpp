...
bool friction_controller::cyclic_torque_control(k_api::Base::BaseClient* base, k_api::BaseCyclic::BaseCyclicClient* base_cyclic, k_api::ActuatorConfig::ActuatorConfigClient* actuator_config)
{
    ...
    try
    {
        ...
        {
            iteration_limit = dynamic_data_velocity_values.size();
        }
        ...
        for (int iteration_counter = 0; iteration_counter < iteration_limit; iteration_counter++)
        {
            ...
            if (get_dynamic_values)
            {
                ...
                theta_dot_desired = dynamic_data_velocity_values[iteration_counter];
                // cout<< "theta_dot_desired"<<theta_dot_desired<<endl;
            ...
            while (total_time_sec.count() / SECOND < task_time_limit_sec)
            {
                ...
                for (unsigned int i = 0; i < ACTUATOR_COUNT; i++)
                {
                    ...
                    {
                        if (i != TEST_JOINT) base_command.mutable_actuators(i)->set_position(friction_configuration_SDP[i]);
                    }
                }
                ...
                if (start_test && !cool_down)
                {
                    ...
                    else
                    {
                        if (!equal(0.0,jnt_velocity_vec(TEST_JOINT), breakaway_torque_velocity_threshold)){
                            ...
                            // printf("torcque opint %f \n", jnt_ctrl_torque_vec(TEST_JOINT));
                        }
                    }
                }
                else if (!start_test && !cool_down ) // Stop the motion of the joint before new trial starts
                {
                    // Error calc
                    ...
                    jnt_ctrl_torque_vec(TEST_JOINT) += 0.0009 * (error - previous_error) / DT_SEC; // D term
                    previous_error = error;
                    // cout<<"in else if "<< jnt_velocity_vec(TEST_JOINT)<<endl;
                ...
                {
                    // printf("in else  \n", iteration_counter);
                    if (std::chrono::duration<double, std::micro>(loop_start_time - control_start_time_sec).count() > 5000000)
                    {
                        cool_down = false;
                        start_test = false;
            ...
            if (system_error_occured == true) break; 
            //function to save data to csv file    
            if (get_static_torque) data_collector_obj.create_static_torque_value_file();
            // cout<<"dynamic file"<<endl;
            if (get_dynamic_values) data_collector_obj.save_dynamic_data(dynamic_data_velocity_values[iteration_counter]);
