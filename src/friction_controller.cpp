...
bool friction_controller::example_cyclic_torque_control(k_api::Base::BaseClient* base, k_api::BaseCyclic::BaseCyclicClient* base_cyclic, k_api::ActuatorConfig::ActuatorConfigClient* actuator_config)
{
    ...
    try
    {
        ...
        while (total_time_sec.count() / SECOND < task_time_limit_sec)
        {
            ...
            if(get_static_torque){
                bool start_test_rec,break_loop_rec;
                double jnt_ctrl_torque_vec_rec,jnt_velocity_vec_rec,error_rec,previous_error_rec;

                tie (start_test_rec,jnt_ctrl_torque_vec_rec,jnt_velocity_vec_rec,error_rec,previous_error_rec,break_loop_rec) =data_collector_obj.get_static_torques_values(start_test,jnt_ctrl_torque_vec(TEST_JOINT),jnt_velocity_vec(TEST_JOINT)
                ,error,previous_error,theta_dot_desired,nominal_vel_vec(TEST_JOINT),DT_SEC);

                start_test=start_test_rec;
                jnt_ctrl_torque_vec(TEST_JOINT)=jnt_ctrl_torque_vec_rec;
                jnt_velocity_vec(TEST_JOINT)=jnt_velocity_vec_rec;
                error=error_rec;
                previous_error=previous_error_rec;
                bool break_loop=break_loop_rec;
                if(break_loop){
                    break;
                }
            }
